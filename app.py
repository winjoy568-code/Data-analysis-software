...
def generate_word_report(df, summary_agg, figures, texts, analysis_scope):
    ...
    def safe_add_image(key, title):
        ...
        if key in figures:
            try:
                ...
                doc.add_paragraph("[註：此圖表無法在當前雲端環境生成，請參閱網頁版]")
    ...
    doc.add_paragraph(texts.get('unit_insight', ''))
...
with col_btn2:
    if data_ready:
        ...
        fig_rank.update_layout(
            ...
            height=400, font=dict(size=14, color='black', family='Microsoft JhengHei, Noto Sans CJK TC, sans-serif')
        )
        figures['rank'] = fig_rank
        texts['rank_insight'] = f"{summary_agg.iloc[0][group_col]} 表現最佳。{summary_agg.iloc[-1][group_col]} 效率最低。"
        ...
        if len(df) > 1:
            ...
            figures['cv'] = fig_cv
            texts['cv_insight'] = f"{cv_data.iloc[0][group_col]} 最穩定 (CV最低)。"
        ...
        try:
            ...
            fig_corr.update_layout(
                ...
                height=500, font=dict(size=14, color='black', family='Microsoft JhengHei, Noto Sans CJK TC, sans-serif')
            )
            figures['corr'] = fig_corr
            texts['corr_insight'] = "理想落點為右下角。左上角代表空轉浪費。"
        except: pass
        ...
        fig_pie.update_layout(font=dict(size=14, color='black', family='Microsoft JhengHei, Noto Sans CJK TC, sans-serif'))
        figures['pie'] = fig_pie
...
if start_analysis and data_ready:
    with st.spinner('正在分析數據...'):
        ...
        st.plotly_chart(figures['rank'], use_container_width=True)

        # 2. 趨勢
        st.header("2. 生產趨勢與穩定性分析")
        st.subheader("生產穩定度 (CV變異係數)")
        if 'cv' in figures:
            st.plotly_chart(figures['cv'], use_container_width=True)
        ...
        if 'corr' in figures:
            st.plotly_chart(figures['corr'], use_container_width=True)
        ...
        with cp2:
            st.subheader("平均單位能耗")
            st.plotly_chart(figures['unit'], use_container_width=True)
