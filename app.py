...
# ==========================================
# 0. 系統設定與 CSS (System Configuration)
# ==========================================
...
class DataEngine:
    ...
    def clean_and_process(df_raw, params):
        ...
        if not all(col in df.columns for col in required_cols):
            return None, None, f"缺少欄位，請檢查是否包含: {required_cols}"
...
class InsightEngine:
    ...
    def generate_narrative(df, summary_agg, group_col, params):
        ...
        if best_machine['平均單位能耗'] > 0:
            eff_gap = ((worst_machine['平均單位能耗'] - best_machine['平均單位能耗']) / best_machine['平均單位能耗']) * 100
            
        texts['benchmark_analysis'] = f"""
        * **標竿設備 ({best_machine[group_col]})**：表現最佳，平均 OEE 達 **{best_machine['OEE']:.1%}**，為本次分析之冠軍機台。
        * **瓶頸設備 ({worst_machine[group_col]})**：表現最弱，單位生產成本比標竿高出 **{eff_gap:.1f}%**，是主要的成本浪費來源。
        """
...
class VizEngine:
    ...
    def plot_rank(summary_agg, group_col):
        ...
        fig.update_traces(marker_color='#1f618d', texttemplate='%{text:.1%}', textposition='outside')
    ...
    def plot_cv(df, group_col):
        ...
        fig.update_traces(marker_color='#922b21', texttemplate='%{text:.1f}%', textposition='outside')
        fig.update_layout(VizEngine._common_layout())
...
def main():
    ...
    if uploaded_file:
        # 使用 Pandas 直接讀取，若格式正確則存入 Session
        try:
